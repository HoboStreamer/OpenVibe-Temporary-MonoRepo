...
function build2dWorldRouter({ eventBus, realtime, config }) {
    ...
    r.post('/worlds', json, (req, res) => {
        try {
            ...
            const body = req.body || {};
            if (Array.isArray(body.legacy_entities) && body.legacy_entities.length) {
                return res.status(400).json({
                    error: 'legacy 2D World imports are no longer supported',
                    reason: 'sourcevibe_only',
                });
            }
            ...
            if (Array.isArray(body.runtime_entities)) worldStore.setRuntimeEntities(world.id, body.runtime_entities);
