...
async function main() {
    ...
    const logger = createLogger('export-hobostreamer');
    ...
    if (!source.dbPath) {
        throw new Error('Unable to resolve HoboStreamer SQLite path. Provide --db or --legacy-source-root/--legacy-root.');
    }
    if (!source.legacyRoot) {
        throw new Error('Unable to resolve HoboStreamer legacy root. Provide --legacy-root or --legacy-source-root.');
    }
    ...
    logger.info(`Missing media references: ${manifest.diagnostics.missing_media_references || 0}`);
}

main().catch((error) => {
    console.error(`[export-hobostreamer] ❌ ${error.message}`);
    process.exit(1);
});
