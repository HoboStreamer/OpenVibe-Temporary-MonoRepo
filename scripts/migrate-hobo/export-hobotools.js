...
async function main() {
    ...
    const source = resolveLegacySource('hobotools', {
    ...
    if (!source.dbPath) {
        throw new Error('Unable to resolve hobo-tools SQLite path. Provide --db or --legacy-source-root/--legacy-root.');
    }
    if (!source.legacyRoot) {
        throw new Error('Unable to resolve hobo-tools legacy root. Provide --legacy-root or --legacy-source-root.');
    }
