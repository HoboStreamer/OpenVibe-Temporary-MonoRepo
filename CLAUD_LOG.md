...
## Session 30 — 2026-05-25 — Postgres migration drift fix (chat_bans)

### What was done

**Problem:** GPT5.5 repo analysis identified a schema drift failure for `chat_bans`. The drift checker translates the SQLite schema to Postgres and injects `rowid BIGINT GENERATED BY DEFAULT AS IDENTITY UNIQUE` for every table — but the Postgres migration for `chat_bans` was missing that column. We introduced `chat_bans` in session 28/29 and didn't add the rowid to the migration at the time.

**Fix:** Added `rowid BIGINT GENERATED BY DEFAULT AS IDENTITY UNIQUE` as the first column in the `chat_bans` table in `services/openvibe-chat/server/migrations/postgres/001_init.sql`, matching the pattern used by every other table in that file.

### Files changed

| File | Change |
|---|---|
| `services/openvibe-chat/server/migrations/postgres/001_init.sql` | Added `rowid` column to `chat_bans` table |
