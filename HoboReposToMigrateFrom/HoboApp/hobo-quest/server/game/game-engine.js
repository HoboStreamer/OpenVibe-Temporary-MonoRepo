...
// Fire-and-forget cosmetic unlock on hobostreamer (server-to-server)
const cosmeticsSystem = {
    unlockCosmetic(userId, itemId) {
        // Non-blocking — don't wait for result; just fire the request
        const body = JSON.stringify({ userId, itemId });
        const req = http.request('http://127.0.0.1:3000/api/cosmetics/internal-unlock', {
            method: 'POST',
            headers: {
...
// ── Init ─────────────────────────────────────────────────────
function initGameDb() {
    // Pre-schema migration: rename game_structures columns x→tile_x, y→tile_y if needed
    try {
        const cols = db.getDb().pragma('table_info(game_structures)').map(c => c.name);
        if (cols.includes('x') && !cols.includes('tile_x')) {
            db.getDb().exec('ALTER TABLE game_structures RENAME COLUMN x TO tile_x');
            db.getDb().exec('ALTER TABLE game_structures RENAME COLUMN y TO tile_y');
            console.log('[HoboGame] Migrated game_structures columns: x→tile_x, y→tile_y');
        }
        // Rename placed_at → built_at if it exists
        if (cols.includes('placed_at') && !cols.includes('built_at')) {
            try { db.getDb().exec('ALTER TABLE game_structures RENAME COLUMN placed_at TO built_at'); } catch {}
        }
        // Add hp/max_hp columns if missing
        if (!cols.includes('hp')) {
            try { db.getDb().exec('ALTER TABLE game_structures ADD COLUMN hp INTEGER DEFAULT 100'); } catch {}
            try { db.getDb().exec('ALTER TABLE game_structures ADD COLUMN max_hp INTEGER DEFAULT 100'); } catch {}
        }
    } catch { /* table doesn't exist yet — schema.sql will create it */ }

    // Pre-schema migration: rename game_farm_plots.seed_id → crop_id if needed
    try {
        const cols = db.getDb().pragma('table_info(game_farm_plots)').map(c => c.name);
        if (cols.includes('seed_id') && !cols.includes('crop_id')) {
            db.getDb().exec('ALTER TABLE game_farm_plots RENAME COLUMN seed_id TO crop_id');
            console.log('[HoboGame] Migrated game_farm_plots column: seed_id→crop_id');
        }
        if (!cols.includes('fertilized')) {
            try { db.getDb().exec('ALTER TABLE game_farm_plots ADD COLUMN fertilized INTEGER DEFAULT 0'); } catch {}
        }
    } catch { /* table doesn't exist yet */ }

    // Pre-schema migration: game_achievements.unlocked_at → completed_at
    try {
        const cols = db.getDb().pragma('table_info(game_achievements)').map(c => c.name);
        if (cols.includes('unlocked_at') && !cols.includes('completed_at')) {
            db.getDb().exec('ALTER TABLE game_achievements RENAME COLUMN unlocked_at TO completed_at');
            console.log('[HoboGame] Migrated game_achievements: unlocked_at → completed_at');
        }
    } catch { /* table doesn't exist yet */ }

    // Pre-schema migration: game_daily_quest_claims.tier → quest_id
    try {
        const cols = db.getDb().pragma('table_info(game_daily_quest_claims)').map(c => c.name);
        if (cols.includes('tier') && !cols.includes('quest_id')) {
            db.getDb().exec('ALTER TABLE game_daily_quest_claims RENAME COLUMN tier TO quest_id');
            console.log('[HoboGame] Migrated game_daily_quest_claims: tier → quest_id');
        }
    } catch { /* table doesn't exist yet */ }

    // Pre-schema migration: game_daily_quest_progress add updated_at if missing
    try {
        const cols = db.getDb().pragma('table_info(game_daily_quest_progress)').map(c => c.name);
        if (cols.length && !cols.includes('updated_at')) {
            db.getDb().exec('ALTER TABLE game_daily_quest_progress ADD COLUMN updated_at DATETIME DEFAULT CURRENT_TIMESTAMP');
            console.log('[HoboGame] Migrated game_daily_quest_progress: added updated_at');
        }
    } catch { /* table doesn't exist yet */ }

    // Pre-schema migration: game_fish_collection column renames
    try {
        const cols = db.getDb().pragma('table_info(game_fish_collection)').map(c => c.name);
        if (cols.includes('count') && !cols.includes('times_caught')) {
            db.getDb().exec('ALTER TABLE game_fish_collection RENAME COLUMN count TO times_caught');
            console.log('[HoboGame] Migrated game_fish_collection: count → times_caught');
        }
        if (cols.includes('best_weight') && !cols.includes('max_weight')) {
            db.getDb().exec('ALTER TABLE game_fish_collection RENAME COLUMN best_weight TO max_weight');
            console.log('[HoboGame] Migrated game_fish_collection: best_weight → max_weight');
        }
    } catch { /* table doesn't exist yet */ }
...
function getLeaderboard(boardType, limit = 20) {
    ...
    if (!col) return [];
    return db.all(`SELECT gp.user_id, gp.display_name, gp.display_name AS username, ${col} as score, gp.name_effect, gp.particle_effect
        FROM game_players gp ORDER BY ${col} DESC LIMIT ?`, [limit]);
}
