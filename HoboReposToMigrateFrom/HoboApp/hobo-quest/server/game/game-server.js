...
// Fetch cosmetic profile from hobostreamer (server-to-server on localhost)
function getCosmeticProfile(userId) {
    return new Promise((resolve) => {
        const req = http.request(`http://127.0.0.1:3000/api/cosmetics/equipped/${userId}`, {
            method: 'GET',
            timeout: 2000,
        }, (res) => {
            let chunks = '';
...
class GameServer {
    ...
    async _onConnection(ws, req) {
        ...
        // Register lifecycle handlers early so heartbeat works even if init fails
        ws.isAlive = true;
        ...
        try {
            ...
            const cosmeticProfile = await getCosmeticProfile(userId);
            ...
            console.error(`[HoboGame WS] Init failed for ${username} (${userId}):`, err.message);
            this._send(ws, { type: 'error', message: 'Failed to initialize game session' });
        }
    }
    ...
    _handleChat(ws, msg) {
        ...
        // Game chat bridge — hobo-quest doesn't have a global chat server
        // In the future, this could POST to hobostreamer's chat API
    }
