...
// isStaff adapted for hobo-quest JWT roles
function isStaff(user) { if (!user) return false; const r = user.role; return r === 'admin' || r === 'global_mod' || r === 'mod'; }
...
class CanvasService extends EventEmitter {
    ...
    initDb() {
        ...
        // canvas_settings: add type + updated_at if missing
        const settingsCols = rawDb.pragma('table_info(canvas_settings)').map(c => c.name);
        if (!settingsCols.includes('type')) {
            rawDb.exec(`ALTER TABLE canvas_settings ADD COLUMN type TEXT DEFAULT 'json'`);
            rawDb.exec(`ALTER TABLE canvas_settings ADD COLUMN updated_at DATETIME DEFAULT CURRENT_TIMESTAMP`);
            console.log('[Canvas] Migrated canvas_settings: added type, updated_at columns');
        }

        // canvas_bans: rename old columns to match new schema
        const banCols = rawDb.pragma('table_info(canvas_bans)').map(c => c.name);
        if (banCols.includes('ip') && !banCols.includes('ip_address')) {
            rawDb.exec(`ALTER TABLE canvas_bans RENAME COLUMN ip TO ip_address`);
            console.log('[Canvas] Migrated canvas_bans: ip → ip_address');
        }
        if (banCols.includes('banned_by') && !banCols.includes('created_by')) {
            rawDb.exec(`ALTER TABLE canvas_bans RENAME COLUMN banned_by TO created_by`);
            console.log('[Canvas] Migrated canvas_bans: banned_by → created_by');
        }
        if (banCols.includes('ban_type') && !banCols.includes('action_type')) {
            rawDb.exec(`ALTER TABLE canvas_bans RENAME COLUMN ban_type TO action_type`);
            console.log('[Canvas] Migrated canvas_bans: ban_type → action_type');
        }

        // canvas_tiles: add missing columns, rename placed_at → updated_at
        const tileCols = rawDb.pragma('table_info(canvas_tiles)').map(c => c.name);
        if (!tileCols.includes('username')) {
            rawDb.exec(`ALTER TABLE canvas_tiles ADD COLUMN username TEXT`);
        }
        if (!tileCols.includes('ip_address')) {
            rawDb.exec(`ALTER TABLE canvas_tiles ADD COLUMN ip_address TEXT`);
        }
        if (tileCols.includes('placed_at') && !tileCols.includes('updated_at')) {
            rawDb.exec(`ALTER TABLE canvas_tiles RENAME COLUMN placed_at TO updated_at`);
            console.log('[Canvas] Migrated canvas_tiles: placed_at → updated_at, added username/ip_address');
        }
    ...
    sanitizeAction(row, { includeIp = false } = {}) {
        ...
        const action = {
            ...
            x: row.x ?? null,
            y: row.y ?? null,
            prev_color_index: row.prev_color_index != null ? Number(row.prev_color_index || 0) : (row.prev_color != null ? Number(row.prev_color || 0) : null),
            color_index: row.color_index != null ? Number(row.color_index || 0) : (row.new_color != null ? Number(row.new_color || 0) : null),
            user_id: row.user_id ?? null,
            username: row.username || null,
    ...
    getActiveRegions() {
        return db.all(`
            SELECT crl.*, crl.locked_by AS created_by
    ...
    getDerivedCooldown(user) {
        ...
        const isAnon = !user?.id || user?.is_anon;
        const totalLevel = user?.id && !isAnon ? (game.getPlayer(user.id)?.total_level || 0) : 0;
        ...
        // Anon users get a 3x cooldown penalty (minimum 30 seconds)
        if (isAnon) {
            cooldownSeconds = Math.max(30, cooldownSeconds * 3);
        }
        ...
        const placementsPerMinute = override?.placements_per_minute != null ? Number(override.placements_per_minute) : Math.max(isAnon ? 1 : 3, Math.floor(60 / Math.max(1, cooldownSeconds)));
        return { total_level: totalLevel, cooldown_seconds: cooldownSeconds, placements_per_minute: placementsPerMinute, override, is_anon: isAnon };
    }
