...
// Phase 2 (OpenVibe): additive multi-issuer verification. When
// OPENVIBE_AUTH_URL is unset this module is inert and behaviour is unchanged.
const openvibeIssuer = require('./openvibe-issuer');

/**
 * Verify a hobo.tools RS256 JWT token (or, when federated, an OpenVibe one).
 * Returns decoded payload or null.
 */
function verifyToken(token) {
    if (!token) return null;

    // Cheap iss peek to route to the correct verifier — never throws.
    let iss = null;
    try {
        const decoded = jwt.decode(token, { complete: false });
        if (decoded && typeof decoded === 'object') iss = decoded.iss || null;
    } catch { /* fall through */ }

    if (iss && openvibeIssuer.isOpenVibeIssuer(iss)) {
        return openvibeIssuer.verifyOpenVibe(token);
    }
