...
function getAllowedOrigins() {
    ...
    // OpenVibe network (Phase 2): when the OpenVibe surfaces are configured,
    // allow them too so the new domains can hit this service during the
    // federation period. No-op when OPENVIBE_*_URL env vars are unset.
    [
        process.env.OPENVIBE_NETWORK_URL,
        process.env.OPENVIBE_AUTH_URL,
        process.env.OPENVIBE_API_URL,
        process.env.OPENVIBE_ADMIN_URL,
        process.env.OPENVIBE_MY_URL,
        process.env.OPENVIBE_THEMES_URL,
    ].forEach(u => {
